...
import { MockCozyToken, ICurvePool, IERC20, CurveThreeTokens } from '../typechain';
...
const forkedChainId = getChainId(hre) as keyof typeof chainAddresses.curve;

// Addresses for Curve contracts and tokens, per chain
...
// Define the balanceOf mapping slot number to use for finding the slot used to store balance of a given address, per chain id
const chainTokenBalanceOfSlots = {
  1: { usdt: '0x2', wbtc: '0x0', weth: '0x3' },
  42161: { usdt: '0x33', wbtc: '0x33', weth: '0x33' },
};

// Define the totalSupply mapping slot number to use for finding the slot used to store the totalSupply of the 3crypto LP token, per chain id
const chainCurveTokenTotalSupplySlots = {
  1: '0x4',
  42161: '0x9',
};

// Addresses on the chain forked by hardhat
const curveAddresses = chainAddresses.curve[forkedChainId];
const tokenAddresses = chainAddresses.tokens[forkedChainId];

// Token balanceOf slots on the chain forked by hardhat
const tokenBalanceOfSlots = chainTokenBalanceOfSlots[forkedChainId];
// totalSupply slot for Curve token on the chain forked by hardhat
const curveTokenTotalSupplySlot = chainCurveTokenTotalSupplySlots[forkedChainId];
...
describe('Curve3Crypto', function () {
  ...
  async function setupFixture() {
    ...
    const CurveThreeTokensArtifact = await artifacts.readArtifact('CurveThreeTokens');
    const trigger = <CurveThreeTokens>await deployContract(deployer, CurveThreeTokensArtifact, triggerParams);
